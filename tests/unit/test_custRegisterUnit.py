...
def testCustRegister(testClient):
    ...
    customer = Customer(**requestData)
    
    # Encode the password into bytes to be hashed, using same format as the HTML form allows
    passBytes = customer.password.encode('utf-8', 'strict')
    hashedPass = bcrypt.hashpw(passBytes, salt)

    # Encode the card number in bytes to be hashed
    cardBytes = customer.cardNum.encode('utf-8', 'strict')
    hashedCard = bcrypt.hashpw(cardBytes, salt)
    
    # Start checking data
    assert type(customer.fname) == str
    assert type(customer.lname) == str
    assert dateCheck(customer.dob)
    assert emailCheck(customer.email)
    assert phoneCheck(customer.phone)
